...
import { UomCategoryModel } from '../../models/master/uom-category.model';
...
export class UomCategoryService {
  ...
  get(): Observable<RootModel> {
    const url = `${environment.main_url}/master/uom-categories`;
    return this.http.get<RootModel>(url);
  }

  getById(id: number): Observable<RootModel> {
    const url = `${environment.main_url}/master/uom-categories/${id}`;
    return this.http.get<RootModel>(url);
  }
  ...
  update(payload: Partial<UomCategoryModel>): Observable<RootModel> {
    const url = `${environment.main_url}/master/uom-categories/${payload.id}`;
